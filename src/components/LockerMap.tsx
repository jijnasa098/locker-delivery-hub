...
const LockerMap = ({ 
  ...
  const organizeLockersByGrid = () => {
    // Only create the grid if we have lockers
    if (lockers.length === 0) return [];

    // Create an empty grid matrix based on the largest row and column values found in lockers
    let maxRow = 0;
    let maxCol = 0;

    // Find the maximum row and column based on existing lockers
    lockers.forEach(locker => {
      if (locker.row !== undefined && locker.column !== undefined) {
        maxRow = Math.max(maxRow, locker.row);
        maxCol = Math.max(maxCol, locker.column);
      }
    });

    // Use the provided rows/columns values or calculated ones
    const actualRows = Math.max(rows, maxRow + 1);
    const actualCols = Math.max(columns, maxCol + 1);
    
    const grid: (Locker | null)[][] = Array(actualRows).fill(null).map(() => Array(actualCols).fill(null));
    
    lockers.forEach(locker => {
      // If locker has explicit row/column values, use those
      if (locker.row !== undefined && locker.column !== undefined) {
        if (locker.row < actualRows && locker.column < actualCols) {
  ...
  // If there are no lockers, return an empty div
  if (lockers.length === 0) {
    ...
    <div className="w-full">
      ...
      <div className="bg-secondary/50 rounded-lg p-4">
        {lockerGrid.map((row, rowIndex) => {
          // Check if this row has any lockers, skip if hideEmptySlots and row is empty
          const hasLockers = row.some(cell => cell !== null);
          if (hideEmptySlots && !hasLockers) return null;
          
          return (
            <div key={`row-${rowIndex}`} className="flex mb-4 last:mb-0">
              {row.map((locker, colIndex) => {
                // Skip rendering empty slots if hideEmptySlots is true
                if (hideEmptySlots && locker === null) return null;
